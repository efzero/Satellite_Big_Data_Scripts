...
class par_utils:
	...
	def __init__():
		...
		data = {}
		ds = nc.Dataset(self.filepath, mode='r')
...
#given 4 lon and lat corners, find the according weighted average pars
	def get_weighted_par(self, lon_corners, lat_corners, time_ind):
		...
		ret_= 0
		gome_box = Polygon(np.vstack(lon_corners, lat_corners).T)
		...
		for lon in range(left, right, 1):
			for lat in range(down, up, 1):
				...
				weight = gome_box.intersection(par_box).area
				ret_ += weight * self.data['total_par'][time_ind][lon_ind, lat_ind]
		return ret_


    #input integer values of lon and lat, representing the left, down corner of a par grid
	def get_par_lon_ind(lon):
		return lon + 360 - 263

	def get_par_lat_ind(lat):
		return lat -37

	def get_time_ind(cur_date, hour):
