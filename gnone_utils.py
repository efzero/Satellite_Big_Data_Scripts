...
class GNOME_utils:
	...
	def load_gnome(self, path, date_):
		...
		filtered_lon_ind = np.where((data_lons >= self.min_lon + 0.8)&(data_lons <= self.max_lon - 0.8))
		filtered_lat_ind = np.where((data_lats >= self.min_lat + 0.3)&(data_lats <= self.max_lat - 0.3))
		...
		self.lons = data_lons[indices]
		self.lats = data_lats[indices]
		self.GOME_sif = data_sifs[indices]
		return True
